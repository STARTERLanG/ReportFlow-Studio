...
def gen_v1_linear():
    ...
    # 连线
    new_edges = [
        {"id": "e1", "source": "start", "target": "preprocess", "sourceHandle": "source", "targetHandle": "target", "type": "custom"},
        {"id": "e2", "source": "preprocess", "target": "end", "sourceHandle": "source", "targetHandle": "target", "type": "custom"}
    ]
...
def gen_v2_simple_router():
    ...
    # Start
    nodes.append({
        "id": "start", "type": "custom", "position": {"x": 0, "y": 0},
        "data": {"type": "start", "title": "Start", "variables": [{"name": "q", "type": "string"}]}
    })
    # Router (引用 Start)
    nodes.append({
        "id": "router", "type": "custom", "position": {"x": 200, "y": 0},
        "data": {
            "type": "if-else", "title": "Router",
            "cases": [
                {
                    "case_id": "true", "logical_operator": "and",
                    "conditions": [{"id": "c1", "variable_selector": ["start", "q"], "comparison_operator": "contains", "value": "a", "varType": "string"}]
                }
            ]
        }
    })
    # End True
    nodes.append({
        "id": "end_true", "type": "custom", "position": {"x": 400, "y": -100},
        "data": {"type": "end", "title": "End A", "outputs": []}
    })
    # End False
    nodes.append({
        "id": "end_false", "type": "custom", "position": {"x": 400, "y": 100},
...
def gen_v3_code_outputs():
    ...
    for n in nodes:
        if n["data"]["type"] == "code":
            ...
            # 同时检查一下 title，防止缺失
            if "title" not in n["data"]: n["data"]["title"] = "Code Node"
