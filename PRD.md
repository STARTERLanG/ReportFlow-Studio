...
### 3.2 可视化审查界面 (Pending)
- *当前状态*：已实现 NiceGUI 基础界面。
- *目标*：支持 Mermaid 流程图实时渲染，以及历史记录一键回溯。

### 3.3 提示词质量自迭代机制 (Planned)
- *当前状态*：PromptExpert 仅执行一次性优化，缺乏闭环。
- *目标*：
  - **Reviewer 节点**：引入专门的提示词评估智能体，对 Prompt 的专业度、安全性、指令清晰度进行打分。
  - **反馈循环**：若评分低于阈值，将具体改进建议回传至 `PromptExpert` 进行二次重写。
  - **动态步进**：通过状态控制（而非简单的 plan 裁剪）来决定是否进入下一步骤。
