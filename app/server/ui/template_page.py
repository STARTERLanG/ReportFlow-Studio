...
def render_template_page():
        ...
        try:
            with Session(engine) as session:
                statement = (
                    ...
                    .where(WorkflowHistory.category == "template-parse")
                            ...
                            with ui.row().classes("w-full items-center justify-between"):
                                with ui.row().classes("gap-3"):
                                    ...
                                    ui.label(record.created_at.strftime("%H:%M")).classes(
                                        "text-[10px] font-bold text-teal-400 uppercase tracking-tighter"
                                    )
                    ...
                    try:
                        ...
                        try:
                            ...
                            result = await loop.run_in_executor(
                                None, template_service.parse_and_decompose, tmp_path, filename
                            )
        ...
        with ui.dialog() as dialog, ui.card().classes("w-full max-w-2xl p-6"):
            ...
            with ui.column().classes("w-full gap-4"):
                ...
                type_select = ui.select(
                    {"extraction": "数据提取", "generation": "智能写作"}, value=task["type"], label="任务类型"
                ).classes("w-full")

                desc_input = (
                    ui.textarea("任务描述", value=task.get("description", "")).classes("w-full").props("autogrow")
                )
                req_input = (
                    ui.textarea("执行要求", value=task.get("requirements", "")).classes("w-full").props("autogrow")
                )
                ...
                def update_content_fields():
                    ...
                    with content_container:
                        if type_select.value == "extraction":
                            ...
                            ui.textarea("待提取字段 (用逗号分隔)", value=current_val).bind_value(
                                task, "fields_temp"
                            ).classes("w-full").props("autogrow")
                        else:
                            ref_str = task.get("reference_content", "") if not is_new else ""
                            current_val = task.get("ref_temp", ref_str)
                            ui.textarea("参考原文内容", value=current_val).bind_value(task, "ref_temp").classes(
                                "w-full"
                            ).props("autogrow")
    ...
    def refresh_ui():
        ...
        with results_container:
            for i, task in enumerate(state["tasks"]):
                ...
                with ui.row().classes("w-full max-w-[880px] no-wrap items-start justify-center"):
                    ...
                    with ui.element("div").classes("task-card-large p-8 flex flex-col gap-6 h-auto relative group"):
                        # 操作按钮组 (悬浮显示)
                        with ui.row().classes(
                            "absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity gap-1"
                        ):
                            with ui.button(icon="edit", on_click=lambda _, idx=i: open_edit_dialog(idx)).props(
                                "flat round color=grey-5 size=sm"
                            ):
                                ui.tooltip("编辑任务")
                            with ui.button(icon="delete", on_click=lambda _, idx=i: delete_task(idx)).props(
                                "flat round color=grey-5 size=sm"
                            ):
                                ui.tooltip("删除任务")
                        ...
                        # Card Header
                        with ui.row().classes(
                            "items-center gap-3 pr-20"
                        ):  # 移除了 w-full justify-between，改为靠左紧凑排列
            ...
            # 底部添加按钮
            with (
                ui.button("添加新任务", icon="add", on_click=lambda: open_edit_dialog(None))
                .classes(
                    "w-full max-w-[880px] h-16 text-lg font-bold border-2 border-dashed border-slate-300 text-slate-400 hover:border-teal-500 hover:text-teal-500 hover:bg-teal-50 transition-colors rounded-2xl mt-4"
                )
                .props("flat")
            ):
                pass
