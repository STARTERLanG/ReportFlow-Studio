...
def dify_yaml_to_mermaid(yaml_str: str) -> str:
    ...
    try:
        ...
        # 1. 定义节点样式
        for node in nodes:
            # 为 ID 添加前缀 'n_' 以避免 'end', 'graph' 等保留字冲突
            node_id = "n_" + str(node["id"]).replace("-", "_")
            title = str(node["data"].get("title", node["id"]))
            ...
            if node_type == "start" or node_type == "end":
                mermaid.append(f"  {node_id}(({safe_title}))")
        ...
        # 2. 定义连线
        for edge in edges:
            source = "n_" + str(edge["source"]).replace("-", "_")
            target = "n_" + str(edge["target"]).replace("-", "_")
