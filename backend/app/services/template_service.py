...
class TemplateService:
    def __init__(self):
        logger.info(
            f"初始化 TemplateService LLM: Model={settings.llm.model_name}, BaseURL={settings.llm.base_url}"
        )
        self.llm = ChatOpenAI(
    ...
    def parse_and_decompose(self, file_path: str) -> List[Dict[str, Any]]:
        ...
        # 2. 调用 AI 进行拆解
        logger.info(
            f"提取文本成功 (长度: {len(full_text)} 字符)，正在进行 AI 任务拆解..."
        )
    ...
    def _decompose_with_llm(self, content: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            return [
                {"task_name": "解析失败", "description": str(e), "requirements": ""}
            ]
